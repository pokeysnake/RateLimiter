...
# confirms that we can fill up the capacity properly and the {bool, float} returns of is_allowed are valid
def test_allows_up_to_capacity():
    r.delete("test_capacity_key")

    bucket = TokenBucket(r)
    for i in range(5):
        allowed, remaining = bucket.is_allowed("test_capacity_key", 5, 0.01, 1)
        assert allowed is True
        assert remaining == 5 - (i + 1)

    allowed_after, remaining_after = bucket.is_allowed("test_capacity_key", 5, 0.01, 1)
...
def test_capacity_then_refill():
    ...
    bucket = TokenBucket(r)
    for i in range(5):
        allowed, remaining = bucket.is_allowed("test_capacity_reject_after", 5, 10, 1)
        assert allowed is True
        assert remaining == 5 - (i + 1)

    time.sleep(5)

    allowed_after, remaining_after = bucket.is_allowed(
        "test_capacity_reject_after", 5, 10, 1
    )
    ...
    with patch("time.time", return_value=fake_now):
        for i in range(5):
            allowed, remaining = bucket.is_allowed(
                "test_capacity_reject_after_mockclock", 5, 10, 1
            )
            assert allowed is True
            assert remaining == 5 - (i + 1)
...
def test_request_more_than_capacity():
    ...
    for i in range(5):
        allowed, remaining = bucket.is_allowed(
            "test_request_more_than_capacity", 5, 1, 10
        )
...
def test_request_greater_one():
    ...
    for i in range(5):
        allowed, remaining = bucket.is_allowed("test_request_greater_than_one", 5, 1, 3)
...
def test_partial_refill():
    ...
    with patch("time.time", return_value=fake_now):
        for i in range(5):
            allowed, remaining = bucket.is_allowed("test_partial_refill", 5, 1, 1)
            assert allowed is True
            assert remaining == 5 - (i + 1)
